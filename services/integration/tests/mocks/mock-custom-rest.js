...
app.all("*", (req, res, next) => {
    ...
    if (req.path === "/evaluations" && req.method === "POST") {
        ...
        if (webhookUrl) {
            ...
            }).catch((err) => console.error("[Custom REST Mock] Webhook error:", err));
        }
        return;
    }
    ...
    if (matchedEndpoint) {
        ...
        Object.keys(response).forEach((key) => {
            ...
            if (typeof response[key] === "string" &&
                ...
                response[key] = response[key].replace("{timestamp}", Date.now().toString());
            }
        });
        res.json(response);
    }
...
app.use((req, res) => {
    res.status(404).json({
        error: "Endpoint not configured. Use POST /config/endpoints to add endpoints.",
    });
});
app.listen(PORT, () => {
