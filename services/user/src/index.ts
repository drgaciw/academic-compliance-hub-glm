...
app.get('/users/:id', (c) => {
  ...
  return c.json(successResponse({ id, name: 'John Doe', email: 'john@example.com' }));
});
...
app.post('/users', zValidator('json', z.object({
  ...
  return c.json(successResponse({ id: 'new-user-id', ...data }, { requestId: c.get('requestId') }), 201);
});

// Update user
app.put('/users/:id', zValidator('json', z.object({
  firstName: z.string().min(1).optional(),
  lastName: z.string().min(1).optional(),
  role: z.enum(['STUDENT', 'ADVISOR', 'ADMIN', 'COMPLIANCE_OFFICER']).optional(),
})), async (c) => {
  const id = c.req.param('id');
  const data = c.req.valid('json');
